...
EnvGuard 的核心逻辑完全基于 Unix/POSIX 标准设计，因此具有极佳的跨平台稳定性：

- **macOS (Intel & Apple Silicon):** ✅ 完全支持。支持 Zsh 拦截，并兼容 Homebrew (`/usr/local` 或 `/opt/homebrew`) 与 MacPorts 等架构。
- **Linux (Ubuntu, Debian, CentOS 等):** ✅ 完全支持。支持 Bash/Zsh 拦截，并精准解析 `pyenv` 的垫片 (Shims) 机制。
- **Windows:** ⚠️ 部分支持 (开发中)。目前 `envguard doctor` 与 `envguard audit` 等主动诊断指令可正常运行，但尚未支持 PowerShell / CMD 的被动拦截 (Shell Hook) 功能。建议在 Windows 环境下搭配 WSL2 获得完整体验。
...
> [!NOTE]
> **错位解析：`[CORRUPTED]` 与 `[BAD WRAPPER]` 的差异**
> 两者皆为环境错位，但发生在不同层面：
